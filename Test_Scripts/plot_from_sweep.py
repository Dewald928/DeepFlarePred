...
filename = 'MLP_GS_40f.csv'
pathname = os.path.expanduser(
    '~/Dropbox/_Meesters/figures/MLP/Grid_Search/')
...
def get_wandb_runs():
    ...
    for sweep in sweeps:
        ...
        for run in sweep:
            ...
            # that start with _.
            config_list.append({k: v for k, v in run.config.items() if
                                not k.startswith('_')})
        ...
        extra_df = pd.DataFrame({'Best_Train_TSS': extra_df})
        all_df = pd.concat([name_df, config_df, extra_df, summary_df], axis=1)
...
for dataset in dataset_list:
    for batch_size in batch_list:
        ...
        ax = sns.lineplot(x='learning_rate', y='Best_Train_TSS',
                          data=filter_df, ci=68, color='b', label='Train')
        sns.lineplot(x='learning_rate', y='Best_Validation_TSS',
                          data=filter_df, ci=68, ax=ax, color='g', label='Validation')
        sns.lineplot(x='learning_rate', y='Test_TSS',
                          data=filter_df, ci=68, ax=ax, color='y', label='Test')
        ax.set_xscale("log")
        plt.ylabel('TSS')
        plt.title(f"TSS over learning rate. Scaling strategy: {dataset[4:-1]}. Batch size: {batch_size}")
        plt.savefig(pathname + f"TSS_vs_LR_{dataset[4:-1]}_{batch_size}")
        plt.show()