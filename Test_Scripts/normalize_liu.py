...
standardscaler.fit(pd.concat([df_train.loc[:, sharps + lorentz]],
                             ignore_index=True))
...
minmaxscaler.fit(pd.concat([df_train.loc[:,history_features]],
                             ignore_index=True))

his_train_norm = pd.DataFrame(minmaxscaler.transform(df_train.loc[:,history_features]),
                          index=df_train.loc[:,history_features].index,
                          columns=df_train.loc[:,history_features].columns)
his_val_norm = pd.DataFrame(minmaxscaler.transform(df_val.loc[:,history_features]),
                        index=df_val.loc[:,history_features].index,
                        columns=df_val.loc[:,history_features].columns)
his_test_norm = pd.DataFrame(minmaxscaler.transform(df_test.loc[:,history_features]),
...
normalized_train = pd.concat([df_train.iloc[:, :5], phys_train_norm, his_train_norm],
                             axis=1)
normalized_val = pd.concat([df_val.iloc[:, :5], phys_val_norm, his_val_norm], axis=1)
normalized_test = pd.concat([df_test.iloc[:, :5], phys_test_norm, his_test_norm], axis=1)
